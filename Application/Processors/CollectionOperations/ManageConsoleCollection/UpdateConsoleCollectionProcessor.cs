...
public partial class UpdateConsoleCollectionProcessor : IRequestProcessor
{
    ...
    private async Task<bool> UpdateConsoleAsync(UpdateConsoleRequest request, CancellationToken cts)
    {
        try
        {
            var foundConsole = await _userConsoleRepository.SingleOrDefaultAsync(x => x.Id == request.Id, cts);
            ...
            var newConsole = foundConsole.MapAndFill<ConsoleCollectionItem, UpdateConsoleRequest>(request);
